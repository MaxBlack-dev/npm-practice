...
const { maybeAddSudo, validate, saveProgress } = require('./utils');
...
function handleRetry(context) {
  ...
  try {
    ...
    try {
      console.log(chalk.gray("🌐 Resetting npm registry to official registry..."));
      execSync('npm set registry https://registry.npmjs.org/', { stdio: 'ignore' });
      console.log(chalk.gray("✅ Registry reset to official npm registry"));
    } catch (e) {
      console.log(chalk.yellow("⚠️ Failed to reset npm registry"));
    }
    ...
    try {
      ...
      execSync("lsof -ti :4873 | xargs kill 2>/dev/null || true", { stdio: 'ignore' });
    ...
    try {
      ...
      execSync("rm -rf ~/.config/verdaccio ~/.local/share/verdaccio ~/.verdaccio 2>/dev/null || true", { stdio: 'ignore' });
