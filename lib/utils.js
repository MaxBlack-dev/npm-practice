...
/**
 * Get platform-specific expected command
 */
function getExpectedCommand(task) {
  const isWindows = process.platform === 'win32';
  return (isWindows && task.windowsExpectedCommand) ? task.windowsExpectedCommand : task.expectedCommand;
}

/**
 * Get platform-specific output includes
 */
function getOutputIncludes(task) {
  const isWindows = process.platform === 'win32';
  return (isWindows && task.windowsOutputIncludes) ? task.windowsOutputIncludes : task.outputIncludes;
}
